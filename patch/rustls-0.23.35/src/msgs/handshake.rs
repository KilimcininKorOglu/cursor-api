...
extension_struct! {
    ...
    pub(crate) struct ClientExtensions<'a> {
        ...
        /// Extension ordering strategy
        pub(crate) extension_ordering: ExtensionOrdering,
    }
}

/// Strategy for ordering extensions in ClientHello
#[derive(Clone, Debug)]
pub(crate) enum ExtensionOrdering {
    /// Extensions are ordered pseudo-randomly with optional contiguous groups
    ///
    /// A subset of extensions can be configured to appear contiguously
    /// via the `contiguous_group` field.
    Randomized {
        /// Seed for pseudo-random ordering
        seed: u16,
        /// Extensions that must appear together in sequence
        contiguous_group: Vec<ExtensionType>,
    },

    /// Extensions follow a fixed order for fingerprint simulation
    ///
    /// The vector defines the exact order in which extensions will be encoded.
    /// Only extensions present in both this list and the struct will be output.
    Fixed(Vec<ExtensionType>),
}

impl Default for ExtensionOrdering {
    fn default() -> Self {
        ExtensionOrdering::Randomized {
            seed: 0,
            contiguous_group: Vec::new(),
        }
    }
}
...
impl ClientExtensions<'_> {
    ...
    pub(crate) fn used_extensions_in_encoding_order(&self) -> Vec<ExtensionType> {
        match &self.extension_ordering {
            ExtensionOrdering::Randomized { seed, contiguous_group } => {
                let mut exts = self.order_insensitive_extensions_in_random_order(*seed, contiguous_group);
                exts.extend(contiguous_group);
            ...
            ExtensionOrdering::Fixed(order) => order.clone(),
        }
    }
    ...
    ///   These are encoded in random order, controlled by `seed`,
    ///   and this is the set of extensions returned by this function.
    ///
    /// - Second, extensions named in `contiguous_group`, in the order
    ...
    fn order_insensitive_extensions_in_random_order(
        &self,
        seed: u16,
        contiguous_group: &[ExtensionType],
    ) -> Vec<ExtensionType> {
        ...
        order.retain(|ext| {
            !(matches!(
                ...
                ExtensionType::PreSharedKey
                    | ExtensionType::EncryptedClientHello
                    | ExtensionType::EncryptedClientHelloOuterExtensions
            ) || contiguous_group.contains(ext))
        });

        order.sort_by_cached_key(|new_ext| {
            let seed = ((seed as u32) << 16) | (u16::from(*new_ext) as u32);
