...
fn emit_client_hello_for_retry(
    ...
    exts.extension_ordering = input.hello.extension_ordering.clone();
    ...
    ];
    {
        ...
        use crate::msgs::handshake::{SupportedProtocolVersions, ExtensionOrdering};
        use crate::msgs::base::PayloadU8;
        #[inline(always)]
        fn force_set<T>(v: &mut Option<T>, val: T) { *v = Some(val); }
        #[inline(always)]
        fn replace_if<T>(v: &mut Option<T>, f: impl FnOnce() -> T) {
            if let Some(v) = v {
                *v = f()
            }
        }
        replace_if(&mut chp_payload.extensions.signature_schemes, || {
            vec![
        ...
        replace_if(&mut chp_payload.extensions.named_groups, || {
            vec![
        ...
        replace_if(&mut chp_payload.extensions.supported_versions, || SupportedProtocolVersions {
            tls12: true,
            tls13: true,
        });
        force_set(&mut chp_payload.extensions.renegotiation_info, PayloadU8::empty());
        force_set(
        ...
        chp_payload.extension_ordering = ExtensionOrdering::Fixed(vec![
