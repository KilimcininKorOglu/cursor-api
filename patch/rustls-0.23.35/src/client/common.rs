...
use crate::{
    ...
    msgs::{
        enums::ExtensionType,
        handshake::{
            CertificateChain, DistinguishedName, ExtensionOrdering, ProtocolName, ServerExtensions,
        },
    },
    sign,
    sync::Arc,
...
impl<'a> ServerCertDetails<'a> {
    ...
    pub(super) fn into_owned(self) -> ServerCertDetails<'static> {
        let Self { cert_chain, ocsp_response } = self;
...
pub(super) struct ClientHelloDetails {
    ...
    pub(super) extension_ordering: ExtensionOrdering,
...
impl ClientHelloDetails {
    pub(super) fn new(alpn_protocols: Vec<ProtocolName>, extension_order_seed: u16) -> Self {
        Self {
            ...
            extension_ordering: ExtensionOrdering::Randomized {
                seed: extension_order_seed,
    ...
    pub(super) fn server_sent_unsolicited_extensions(
        ...
        extensions
            .extend(received_exts.unknown_extensions.iter().map(|ext| ExtensionType::from(*ext)));
...
impl ClientAuthDetails {
    pub(super) fn resolve(
        ...
        let acceptable_issuers =
            canames.unwrap_or_default().iter().map(|p| p.as_ref()).collect::<Vec<&[u8]>>();
        ...
        if let Some(certkey) = resolver.resolve(&acceptable_issuers, sigschemes) {
            if let Some(signer) = certkey.key.choose_scheme(sigschemes) {
                ...
                return Self::Verify { certkey, signer, auth_context_tls13, compressor };
            }
        }
