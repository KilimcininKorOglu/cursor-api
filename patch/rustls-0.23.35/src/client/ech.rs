use crate::{
    ...
    ClientConfig, CommonState, EncryptedClientHelloError, Error, PeerIncompatible, PeerMisbehaved,
    ProtocolVersion, Tls13CipherSuite,
    client::tls13,
    crypto::{
        SecureRandom,
        hash::Hash,
        hpke::{EncapsulatedSecret, Hpke, HpkePublicKey, HpkeSealer, HpkeSuite},
    },
    ...
    msgs::{
        ...
        codec::{Codec, Reader},
        enums::{ExtensionType, HpkeKem},
        handshake::{
            ClientExtensions, ClientHelloPayload, EchConfigContents, EchConfigPayload, Encoding,
            EncryptedClientHello, EncryptedClientHelloOuter, ExtensionOrdering,
            HandshakeMessagePayload, HandshakePayload, HelloRetryRequest, HpkeKeyConfig,
            HpkeSymmetricCipherSuite, PresharedKeyBinder, PresharedKeyOffer, Random,
...
impl From<EchConfig> for EchMode {
    fn from(config: EchConfig) -> Self { Self::Enable(config) }
}

impl From<EchGreaseConfig> for EchMode {
    fn from(config: EchGreaseConfig) -> Self { Self::Grease(config) }
}
    ...
    pub fn new(
        ...
        for (_i, config) in ech_configs.iter().enumerate() {
            let contents = match config {
                ...
                EchConfigPayload::Unknown { version: _version, .. } => {
                    warn!("ECH config {} has unsupported version {:?}", _i + 1, _version);
            ...
            for cipher_suite in &key_config.symmetric_cipher_suites {
                ...
                let suite = HpkeSuite { kem: key_config.kem_id, sym: *cipher_suite };
                if let Some(hpke) = hpke_suites.iter().find(|hpke| hpke.suite() == suite) {
                    ...
                    return Ok(Self { config: config.clone(), suite: *hpke });
                }
            }
        }
...
impl EchState {
    pub(crate) fn new(
        ...
        let (enc, sender) = config
            .suite
            .setup_sealer(&config.hpke_info(), &HpkePublicKey(key_config.public_key.0.clone()))?;
    ...
    fn encode_inner_hello(
        ...
        inner_hello.extension_ordering = match &outer_hello.extension_ordering {
            ExtensionOrdering::Randomized { seed, .. } => {
                ExtensionOrdering::Randomized { seed: *seed, contiguous_group: Vec::new() }
            }
            ExtensionOrdering::Fixed(_) => {
                ExtensionOrdering::Randomized { seed: 0, contiguous_group: Vec::new() }
            }
        };
        ...
        if let ExtensionOrdering::Randomized { contiguous_group, .. } =
            &mut inner_hello.extension_ordering
        {
            *contiguous_group = compressed_exts.clone();
        }
        ...
        if let Some(resuming) = resuming.as_ref() {
            ...
            self.early_data_key_schedule =
                Some(tls13::fill_in_psk_binder(resuming, &self.inner_hello_transcript, &mut chp));
        ...
        let padding_len = match &self.inner_name {
            ServerName::DnsName(name) => {
                // name.len() = D
                // max(0, L - D)
                core::cmp::max(0, max_name_len.saturating_sub(name.as_ref().len() as u8) as usize)
            }
            _ => {
                // L + 9
    ...
    fn hello_retry_request_conf(retry_req: &HelloRetryRequest) -> Message<'_> {
        Self::ech_conf_message(HandshakeMessagePayload(HandshakePayload::HelloRetryRequest(
            retry_req.clone(),
        )))
    }
    ...
    fn ech_conf_message(hmp: HandshakeMessagePayload<'_>) -> Message<'_> {
        ...
        Message {
            version: ProtocolVersion::TLSv1_3,
            payload: MessagePayload::Handshake { encoded: Payload::new(hmp_encoded), parsed: hmp },
        }
    }
}
...
mod tests {
    ...
    use crate::{
        enums::CipherSuite,
        msgs::handshake::{Random, ServerExtensions, SessionId},
    };
    ...
    fn server_hello_conf_alters_server_hello_random() {
        ...
        let Message {
            payload: MessagePayload::Handshake { encoded: server_hello_encoded_before, .. },
            ..
        } = &message
        else {
        ...
        let Message {
            payload: MessagePayload::Handshake { encoded: server_hello_encoded_after, .. },
            ..
        } = &message
        else {
            unreachable!("ServerHello is a handshake message");
