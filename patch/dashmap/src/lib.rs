...
pub use set::DashSet;
use std::{collections::hash_map::RandomState, sync::OnceLock};
...
impl<'a, K: Eq + Hash, V> DashMap<K, V, RandomState> {
    ...
    /// ```
    pub fn new() -> Self { DashMap::with_hasher(RandomState::default()) }
...
impl<'a, K, V, S> DashMap<K, V, S> {
    ...
    pub fn into_read_only(self) -> ReadOnlyView<K, V, S> { ReadOnlyView::new(self) }
    ...
    /// reviews.insert("Veloren", "What a fantastic game!");
    /// ```
    pub fn with_hasher(hasher: S) -> Self { Self::with_capacity_and_hasher(0, hasher) }
    ...
    pub fn hasher(&self) -> &S { &self.hasher }
}

impl<'a, K, V, S: BuildHasher> DashMap<K, V, S> {
    /// Hash a given item to produce a usize.
    /// Uses the provided or default HashBuilder.
    pub fn hash_usize<T: Hash + ?Sized>(&self, item: &T) -> usize { self.hash_u64(item) as usize }

    fn hash_u64<T: Hash + ?Sized>(&self, item: &T) -> u64 {
    ...
    /// ```
    pub fn iter(&'a self) -> Iter<'a, K, V> { self._iter() }
    ...
    /// ```
    pub fn iter_mut(&'a self) -> IterMut<'a, K, V> { self._iter_mut() }
    ...
    /// ```
    pub fn retain(&self, f: impl FnMut(&K, &mut V) -> bool) { self._retain(f); }
    ...
    /// assert!(map.is_empty());
    /// ```
    pub fn is_empty(&self) -> bool { self._is_empty() }
    ...
    pub fn alter_all(&self, f: impl FnMut(&K, V) -> V) { self._alter_all(f); }
...
impl<'a, K, V, S: BuildHasher> DashMap<K, V, S> {
    ...
    fn _iter(&'a self) -> Iter<'a, K, V> { Iter::new(self) }

    fn _iter_mut(&'a self) -> IterMut<'a, K, V> { IterMut::new(self) }
    ...
    fn _len(&self) -> usize { self.shards.iter().map(|s| s.read().len()).sum() }

    fn _capacity(&self) -> usize { self.shards.iter().map(|s| s.read().capacity()).sum() }
    ...
    fn _clear(&self) { self._retain(|_, _| false) }
    ...
    fn _is_empty(&self) -> bool { self._len() == 0 }
}
...
impl<'a, K: Eq + Hash, V, S: BuildHasher + Clone> Shl<(K, V)> for &'a DashMap<K, V, S> {
    type Output = Option<V>;

    fn shl(self, pair: (K, V)) -> Self::Output { self.insert(pair.0, pair.1) }
}

impl<'a, K: Eq + Hash, V, S: BuildHasher + Clone, Q> Shr<&Q> for &'a DashMap<K, V, S>
where Q: Hash + Equivalent<K> + ?Sized
{
    type Output = Ref<'a, K, V>;

    fn shr(self, key: &Q) -> Self::Output { self.get(key).unwrap() }
}

impl<'a, K: Eq + Hash, V, S: BuildHasher + Clone, Q> BitOr<&Q> for &'a DashMap<K, V, S>
where Q: Hash + Equivalent<K> + ?Sized
{
    type Output = RefMut<'a, K, V>;

    fn bitor(self, key: &Q) -> Self::Output { self.get_mut(key).unwrap() }
}

impl<'a, K: Eq + Hash, V, S: BuildHasher + Clone, Q> Sub<&Q> for &'a DashMap<K, V, S>
where Q: Hash + Equivalent<K> + ?Sized
{
    type Output = Option<(K, V)>;

    fn sub(self, key: &Q) -> Self::Output { self.remove(key) }
}

impl<'a, K: Eq + Hash, V, S: BuildHasher + Clone, Q> BitAnd<&Q> for &'a DashMap<K, V, S>
where Q: Hash + Equivalent<K> + ?Sized
{
    type Output = bool;

    fn bitand(self, key: &Q) -> Self::Output { self.contains_key(key) }
}

impl<'a, K: Eq + Hash, V: PartialEq, S: BuildHasher + Clone> PartialEq
    for DashMap<K, V, S>
{
    fn eq(&self, other: &Self) -> bool {
        self.len() == other.len()
            && self.iter().all(|r| other.get(r.key()).map_or(false, |ro| r.value() == ro.value()))
    }
}

impl<'a, K: Eq + Hash, V: Eq, S: BuildHasher + Clone> Eq for DashMap<K, V, S> {}
...
impl<K: Eq + Hash, V, S: BuildHasher + Clone> IntoIterator for DashMap<K, V, S> {
    ...
    fn into_iter(self) -> Self::IntoIter { OwningIter::new(self) }
}
...
mod tests {
    ...
    fn test_map_view() {
        ...
        let vegetables: [String; 4] =
            ["Salad".to_string(), "Beans".to_string(), "Potato".to_string(), "Tomato".to_string()];
