...
fn get_machine_id() -> String {
    if cfg!(windows) {
        let output = Command::new("REG")
            ...
            .output()
            .expect("Unable to execute REG command");
        ...
        let output = Command::new("ioreg")
            ...
            .output()
            .expect("Unable to execute ioreg command");
        String::from_utf8_lossy(&output.stdout)
            .lines()
        ...
        let output = Command::new("sh")
            ...
            .output()
            .expect("Unable to get machine-id");
...
fn main() {
    let db_path = if cfg!(windows) {
        let app_data = env::var("APPDATA").unwrap_or_else(|_| {
            let profile = env::var("USERPROFILE").expect("USERPROFILE environment variable not found");
    ...
    match Connection::open(&db_path) {
        Ok(conn) => {
            ...
            match token {
                Ok(token) => {
                    println!("Access token: {}", token.trim());
                    ...
                    println!("System machine-id: {}", sys_machine_id);
        ...
        Err(err) => eprintln!("Unable to open database: {}", err),
    }
}
