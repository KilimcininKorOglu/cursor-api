...
/// # 协议Format
/// ```text
/// [压缩标志 1B][Message长度 4B BE][Message体/压缩数据]
/// ```
/// - **字节0**: 压缩标志 (`0x00`=未压缩, `0x01`=gzip压缩)
/// - **字节1-4**: Message体长度，大端序u32
/// - **字节5+**: 实际Message数据
///
/// # 压缩策略
/// 与`encode_message`相同：小Message不压缩，压缩无效时回退到原始数据
///
/// # Arguments
/// * `message` - 实现了`prost::Message`的protobufMessage
///
