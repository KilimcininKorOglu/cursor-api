...
impl ToolResultBuilder {
    ...
    pub fn build(self) -> String {
        ...
        if self.is_error {
            unsafe { serde_json::to_writer(&mut json, &self.content).unwrap_unchecked() };
            ...
            if let Some(first) = self.content.first() {
                let start = json.len();
                unsafe { serde_json::to_writer(&mut json, first).unwrap_unchecked() };
                let end = json.len() - 1;
            ...
            if self.content.len() > 1 {
                for after in &self.content[1..] {
                    let start = json.len();
                    json.push(b',');
                    unsafe { serde_json::to_writer(&mut json, after).unwrap_unchecked() };
                    let end = json.len() - 1;
                    indexes.push(start..end);
                }
            }
